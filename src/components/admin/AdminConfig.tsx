...
export default function AdminConfig() {
  ...
  const updateStatus = async (status: string) => {
    // SOS pone el booleano en TRUE, los demás en FALSE
    const shouldBeClosed = status === 'forced_closed';

    const { error } = await supabase
      .from('store_config')
      .update({ 
        store_status: status,
        is_closed_manual: shouldBeClosed 
      })
      .eq('id', 1);

    if (!error) {
      setConfig({ ...config, store_status: status, is_closed_manual: shouldBeClosed });
    }
  };
