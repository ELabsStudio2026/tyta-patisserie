...
export default function AdminStoreSettings() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-2 pb-24 space-y-10 animate-in fade-in duration-500">
      ...
      {/* SECCIÓN 1: IDENTIDAD DE EMPRESA (ELabs Studio <> Cliente) */}
      <section className="bg-white p-6 sm:p-10 rounded-[2.5rem] border border-gray-100 shadow-sm relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <Field label="Nombre Legal/Tienda" value={config.store_name} onChange={v => setConfig({...config, store_name: v})} />
          <Field label="Teléfono Empresa" value={config.company_phone} onChange={v => setConfig({...config, company_phone: v})} />
          <Field label="WhatsApp Corporativo" value={config.contact_whatsapp} onChange={v => setConfig({...config, contact_whatsapp: v})} />
          <div className="sm:col-span-2 grid grid-cols-3 gap-4">
            <div className="col-span-2">
              <Field label="Calle" value={config.address_street} onChange={v => setConfig({...config, address_street: v})} />
            </div>
            <Field label="N°" value={config.address_number} onChange={v => setConfig({...config, address_number: v})} />
          </div>
          <Field label="Ciudad" value={config.address_city} onChange={v => setConfig({...config, address_city: v})} />
          <Field label="ID Legajo / Cliente (Próximamente)" value="ID-001 (Tyta Patisserie)" onChange={() => {}} disabled={true} />
        </div>
      </section>

      {/* SECCIÓN 2: IDENTIDAD DE MARCA (Público & Estética) */}
      <section className="bg-[#FDFBF7]/50 p-6 sm:p-10 rounded-[2.5rem] border border-[#EDB2D1]/20 shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <Field label="Instagram User" value={config.instagram_user} onChange={v => setConfig({...config, instagram_user: v})} prefix="@" />
          <Field label="WhatsApp de Pedidos" value={config.whatsapp_number} onChange={v => setConfig({...config, whatsapp_number: v})} />
          <Field label="Email Público" value={config.store_email} onChange={v => setConfig({...config, store_email: v})} />
          ...
          <Field label="TikTok User" value={config.tiktok_user} onChange={v => setConfig({...config, tiktok_user: v})} />
          <Field label="Facebook User" value={config.facebook_user} onChange={v => setConfig({...config, facebook_user: v})} />
        </div>
      </section>

      {/* SECCIÓN 3: GESTIÓN INTERNA (Alertas & Seguridad) */}
      <section className="bg-[#2B4233]/5 p-6 sm:p-10 rounded-[2.5rem] border border-[#2B4233]/10 shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <Field label="Email Destino Alertas" value={config.alert_email_destination} onChange={v => setConfig({...config, alert_email_destination: v})} />
          <Field label="Resend API Key" value={config.resend_api_key} onChange={v => setConfig({...config, resend_api_key: v})} />
          <Field label="Contacto Emergencia" value={config.emergency_contact_name} onChange={v => setConfig({...config, emergency_contact_name: v})} />
          <Field label="Teléfono Emergencia" value={config.emergency_contact_phone} onChange={v => setConfig({...config, emergency_contact_phone: v})} />
        </div>
      </section>
...
function Field({ label, value, onChange, prefix, disabled }: any) {
  return (
