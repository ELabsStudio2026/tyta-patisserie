...
// 1. Agregamos isDisabled a las props
export default function MarketingPopup({ isOpen, onClose, products, onAddToCart, isDisabled }: any) {
              ...
              <div key={`${p.id}-${index}`} className="flex-none w-[170px] sm:w-[210px] md:w-[240px] group flex flex-col h-full">
                ...
                <div className="relative aspect-square rounded-[1.2rem] md:rounded-[2rem] overflow-hidden mb-3 shadow-md border-2 border-white flex-none">
                  <img 
                    src={p.image_url} 
                    alt={p.name} 
                    className={`w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 ${isDisabled ? 'grayscale opacity-70' : ''}`} 
                  />
                </div>
                ...
                <div className="flex flex-col flex-1">
                  ...
                  {/* 2. Botón con lógica de estado y manual de marca */}
                  <button 
                    disabled={isDisabled}
                    onClick={() => onAddToCart(p)} 
                    className={`mt-auto w-full py-2.5 rounded-full text-[8px] font-black uppercase tracking-widest transition-all transform shadow-md
                      ${isDisabled 
                        ? 'bg-gray-200 text-gray-400 cursor-not-allowed' 
                        : 'bg-[#2B4233] text-white hover:bg-[#EDB2D1] hover:text-[#2B4233] active:scale-95'
                      }`}
                  >
                    {isDisabled ? 'Tienda en Pausa' : 'Lo quiero'}
                  </button>
                </div>
              </div>
            ))}
