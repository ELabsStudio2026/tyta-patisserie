...
export default function TiendaPage() {
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      const whatsappNumber = "5491130302451"; 
      ...
      const message = 
        ...
        `https://tytapatisserie.com.ar/pedido/${orderRef}\n\n` +
  ...
  useEffect(() => {
    ...
    const handleScroll = () => { if (scrollContainer) setShowTopButton(scrollContainer.scrollTop > 300); };
  ...
  return (
    <div className="flex flex-col h-screen bg-[#FDFBF7] text-[#2B4233] font-josefin overflow-hidden relative">
      ...
      <div className="fixed bottom-[8vh] right-6 md:right-10 z-[60] flex flex-col gap-3 items-end">
        <a href="https://api.whatsapp.com/send?phone=5491130302451" target="_blank" className="w-14 h-14 md:w-16 md:h-16 bg-[#25D366] rounded-full shadow-2xl flex items-center justify-center hover:scale-110 transition-all">
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#FDFBF7] w-full max-w-2xl rounded-[3rem] overflow-hidden shadow-2xl flex flex-col md:flex-row">
            ...
            <div className="w-full md:w-1/2 aspect-square"><img alt="Tyta Patisserie" src={selectedProduct.image_url || '/images/placeholder.jpg'} className="w-full h-full object-cover" /></div>
            <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col">
              ...
              {selectedProduct.description && (
                <p className="text-sm opacity-70 mb-8 font-josefin italic leading-relaxed border-l-2 border-[#EDB2D1]/30 pl-4">
                  {selectedProduct.description}
                </p>
              )}
              ...
              <div className="mt-auto flex flex-col gap-4">
                ...
                <button onClick={() => { addToCart(selectedProduct); setSelectedProduct(null); }} className="w-full py-4 bg-[#2B4233] text-white rounded-full font-black uppercase text-[11px] tracking-widest hover:bg-[#EDB2D1] hover:text-[#2B4233] transition-all shadow-lg">Añadir al pedido</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="relative w-full max-w-md bg-[#FDFBF7] shadow-2xl flex flex-col h-full border-l-[12px] border-[#EDB2D1]">
                  ...
                  {cart.length === 0 ? <p className="opacity-30 uppercase font-black text-center py-20 tracking-widest">Nada por aquí aún</p> : 
                    cart.map(item => (
                      <div key={item.id} className="flex gap-4 items-center bg-white p-4 rounded-[2rem] shadow-sm border border-[#EDB2D1]/5">
                        <img alt="Tyta Patisserie" src={item.image_url || '/images/placeholder.jpg'} className="w-16 h-16 rounded-2xl object-cover" />
            ...
            {step === 'cart' && cart.length > 0 && (
              <div className="p-8 bg-white border-t border-[#EDB2D1]/10">
                <div className="flex justify-between items-end mb-6"><span className="text-3xl font-black font-mono text-[#2B4233]">${(cartTotal/100).toLocaleString('es-AR')}</span></div>
      ...
      <style jsx global>{` .custom-scrollbar::-webkit-scrollbar { width: 3px; } .custom-scrollbar::-webkit-scrollbar-thumb { background: #EDB2D1; border-radius: 10px; } `}</style>
    </div>
  );
}