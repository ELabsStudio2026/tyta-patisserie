...
const handleFinalSubmit = async () => {
    ...
    try {
      ...
      // 2. CONFIGURACIÓN DEL MENSAJE BOUTIQUE
      const whatsappNumber = "5491132125647"; // Reemplazar con el de Susana
      const detail = cart.map(item => `• ${item.name} (x${item.quantity})`).join('\n');
      
      // Construcción estética del mensaje
      const message = 
