...
export default function AdminPage() {
  ...
  const [mounted, setMounted] = useState(false); // Para evitar el error de Hydration
  ...
  useEffect(() => {
    setMounted(true); // Marcamos que el componente ya está en el navegador
    fetchInitialData();
  }, []);
  ...
  const [newProduct, setNewProduct] = useState<any>({ name: "", price: 0, cost: 0, category: "", image_url: "", stock: 0, critical_stock: 3, is_visible: true });
  ...
  // Si no está montado (en el servidor), mostramos un estado de carga vacío para evitar el error
      ...
      <header className="max-w-[1400px] mx-auto mb-6 flex flex-wrap justify-between items-end bg-white p-6 rounded-[2.5rem] shadow-sm border border-[#EDB2D1]/10 gap-4">
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="flex gap-2">
            ...
            <button onClick={() => { setNewProduct({ name: "", price: 0, cost: 0, category: categories[0]?.name || "", image_url: "", stock: 0, critical_stock: 3, is_visible: true }); setIsAdding(true); }} className="px-5 py-2 bg-[#2B4233] text-white rounded-full text-[9px] font-black uppercase tracking-widest">+ NUEVO</button>
          </div>
              ...
              return (
                <tr key={p.id} className={`transition-colors ${selectedIds.includes(p.id) ? 'bg-[#EDB2D1]/5' : 'hover:bg-[#FDFBF7]/50'}`}>
                  ...
                  <td className="px-2 py-3 flex items-center gap-3">
                    <img alt="Tyta Patisserie" src={p.image_url || '/images/placeholder.jpg'} className="w-10 h-10 rounded-xl object-cover border-2 border-white" alt="" />
          ...
          <div className="relative w-full max-w-xs bg-white shadow-2xl border-l-8 border-[#EDB2D1] flex flex-col h-full overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-2 space-y-4 custom-scrollbar">
              <div className="flex flex-col items-center p-4 border-2 border-dashed border-[#EDB2D1]/20 rounded-[2rem] bg-[#FDFBF7]">
                <img alt="Tyta Patisserie" src={editingProduct?.image_url || newProduct.image_url || "/images/placeholder.jpg"} className="w-20 h-20 rounded-xl object-cover mb-2 border-2 border-white shadow-md" alt="" />
                <CldUploadWidget uploadPreset={process.env.NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET} options={{ maxFileSize: 25000000, clientAllowedFormats: ["jpg", "png", "jpeg", "webp"], maxImageWidth: 2000, compression: 0.7, singleUploadAutoClose: true }}>
